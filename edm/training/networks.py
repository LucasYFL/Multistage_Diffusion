...
@persistence.persistent_class
class MultistageUNet(torch.nn.Module):
    def __init__(self,
        img_resolution,                     # Image resolution at input/output.
        in_channels,                        # Number of color channels at input.
        out_channels,                       # Number of color channels at output.
        ...
        en_model_channels   = 128,          # Base multiplier for the number of encoder channels.
        ...
        assert encoder_type in ['standard', 'skip', 'residual']
        assert decoder_type in ['standard', 'skip']
        assert stage_num == len(de_model_channels)
        assert stage_num == len(stage_interval)

        super().__init__()
        self.label_dropout = label_dropout
        emb_channels = en_model_channels * channel_mult_emb
        noise_channels = en_model_channels * channel_mult_noise
        init = dict(init_mode='xavier_uniform')
        init_zero = dict(init_mode='xavier_uniform', init_weight=1e-5)
        init_attn = dict(init_mode='xavier_uniform', init_weight=np.sqrt(0.2))
        block_kwargs = dict(
        ...
        for stage_idx in range(stage_num):
            ...
            for level, mult in reversed(list(enumerate(channel_mult))):
                res = img_resolution >> level
                if level == len(channel_mult) - 1:
                    dec[f'{res}x{res}_in0'] = UNetBlock(in_channels=cout, out_channels=cout, attention=True, **block_kwargs)
                    dec[f'{res}x{res}_in1'] = UNetBlock(in_channels=cout, out_channels=cout, **block_kwargs)
                else:
                    dec[f'{res}x{res}_up'] = UNetBlock(in_channels=cout, out_channels=cout, up=True, **block_kwargs)
                for idx in range(num_blocks + 1):
                    cin = cout + skips_copy.pop()
                    cout = de_model_channels[stage_idx] * mult
                    attn = (idx == num_blocks and res in attn_resolutions)
                    dec[f'{res}x{res}_block{idx}'] = UNetBlock(in_channels=cin, out_channels=cout, attention=attn, **block_kwargs)
                if decoder_type == 'skip' or level == 0:
                    if decoder_type == 'skip' and level < len(channel_mult) - 1:
                        dec[f'{res}x{res}_aux_up'] = Conv2d(in_channels=out_channels, out_channels=out_channels, kernel=0, up=True, resample_filter=resample_filter)
                    dec[f'{res}x{res}_aux_norm'] = GroupNorm(num_channels=cout, eps=1e-6)
                    dec[f'{res}x{res}_aux_conv'] = Conv2d(in_channels=cout, out_channels=out_channels, kernel=3, **init_zero)
