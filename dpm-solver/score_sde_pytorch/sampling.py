...
def get_dpm_solver_sampler(sde, shape, inverse_scaler, steps=10, eps=1e-3,
    ...
    def dpm_solver_sampler(models, compare_step):
        ...
        with torch.no_grad():
            noise_pred_fns = []
            for m in models:
                noise_pred_fns.append(get_noise_fn(
                    sde, m, train=False, continuous=True))
            dpm_solver = DPM_Solver(noise_pred_fns, ns, compare_step=compare_step, algorithm_type=algorithm_type,
